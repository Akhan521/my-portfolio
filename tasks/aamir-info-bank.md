...
3. **The MLOps port.** Did the complete port of linear performance onto the standardized MLOps framework: **training 109 min to 15 min, full train-and-deploy cycle ~2.5 hours to 27 min, 68% less compute**. Fleet-wide, cheaper per month while serving 50% more models. The speedup came from finding the legacy pipeline was **retraining the model ~13 times per run** to produce output nothing downstream read, then training the production model separately anyway; removing that redundant retraining was the dominant contributor. **Never call this "removing validation"** in copy, it reads as cutting safety checks; say *redundant retraining*. The migration also removed the real bottleneck: starting an experiment used to need an infrastructure change against shared code, and became a branch in a shadow deployment.
